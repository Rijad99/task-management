...
function Modal(props: PropsWithChildren<ModalProps>) {
  ...
  const animateModal = props.isModalShown ? modalVariants.visible : modalVariants.hidden;

  return createPortal(
    <motion.div className={modalCSS.overlay} initial={{ opacity: 0, transform: 'scale(0.9)', pointerEvents: 'none' }} animate={animateModal}>
      <Button size={ButtonSize.MEDIUM} type={ButtonType.PRIMARY} onClick={props.onModalClose} additionalClasses={modalCSS.closeModalButton}>
        <Svg path={plusIcon} width="15" height="13" viewBox="0 0 13 12" color={SvgColors.NEUTRAL} />
      </Button>
