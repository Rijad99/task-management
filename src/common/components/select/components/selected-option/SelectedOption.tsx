...
function SelectedOption({ selectedOption, isSelectOpen, placeholder, selectedOptionAdditionalClasses, onSelectOpen }: SelectedOptionProps) {
    ...
    return (
        <div className={`${selectedOptionCSS.selectedOptionContainer} ${selectedOptionAdditionalClasses ? selectedOptionAdditionalClasses : ''}`} onClick={handleSelectOpen}>
            <span className={`${selectedOptionCSS.option} ${selectedOption && selectedOptionCSS.placeholder}`}>
                ...
                <Svg path={arrowIcon} width='8' height='6' strokeWidth='1.5' viewBox='0 0 12 7' stroke={SvgColors.BLACK} strokeLinecap={SvgStrokeLineCap.ROUND} strokeLinejoin={SvgStrokeLineJoin.ROUND} additionalClasses={`${isSelectOpen ? selectedOptionCSS.rotateArrow : selectedOptionCSS.arrow}`} />
            </span>
        </div>
    )
}
