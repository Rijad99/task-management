...
function useNotificationHook(notification: Notification) {
    ...
    const description = <>
        <span className={notificationCSS.user}>{`${notification.user.firstName} ${notification.user.lastName} `}</span>
    <span>{typeText} </span>
    <span className={notificationCSS.task}>{notification.task}</span>
        </>

    const userInitials = `${notification.user.firstName.substring(0, 1)}${notification.user.lastName.substring(0, 1)}`
    const initialsCircleColor = colors[Math.floor(Math.random() * colors.length)]

    return { localization, description, userInitials, initialsCircleColor }
}
