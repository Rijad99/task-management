...
import { SvgClipRule, SvgColors, SvgFillRule } from '../../../../../common/components/svg/Svg.types';
...
function NotificationItem({ notifications }: NotificationItemProps) {
  const [isNotificationsPopupShown, setIsNotificationsPopupShown] = useState<boolean>(false);

  const newNotifications = notifications.filter((notification) => notification.state === NotificationState.UNREAD);
  ...
  return (
    <div ref={notificationRef} className={notificationItemCSS.notificationItem}>
      <Tooltip
      ...
      >
        ...
        >
          ...
          />
          {notifications.length > 0 && <div className={notificationItemCSS.notificationCircle}></div>}
        </Button>
      </Tooltip>
      <Popup ref={notificationRef} isPopupShown={isNotificationsPopupShown} onOutsideClickPopupClose={() => setIsNotificationsPopupShown(false)}>
